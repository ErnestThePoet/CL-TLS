...
### 身份认证方式
CL-TLS使用基于CLPKC的方案进行身份认证。每个设备均拥有自己的身份ID和密钥对，其中设备密钥对由设备自身生成的部分密钥和KGC生成的部分密钥两部分组成，使用KGC的私钥和设备ID即可验证设备公钥是否属于该设备。

#### 设备注册流程
- 新服务端设备注册流程
  1. 设备选取$Seed_A\gets\{0,1\}^{256}$；
  2. 设备生成部分密钥$(PK_A,SK_A):=\text{Ed25519\_GenKeypair}(Seed_A)$；
  3. 设备将自身身份$ID$和$PK_A$发送给KGC；
  4. KGC选取$Seed_B\gets\{0,1\}^{256}$；
  5. KGC生成部分密钥$(PK_B,SK_B):=\text{Ed25519\_GenKeypair}(Seed_B)$；
...
- 新客户端设备注册流程
  ...
  6. KGC生成部分密钥$(PK_B,SK_B):=\text{Ed25519\_GenKeypair}(Seed_B)$；
...
- KGC密钥对初始化生成流程
  ...
  2. KGC生成部分密钥$(PK_A,SK_A):=\text{Ed25519\_GenKeypair}(Seed_A)$；
  3. KGC选取$Seed_B\gets\{0,1\}^{256}$；
  4. KGC生成部分密钥$(PK_B,SK_B):=\text{Ed25519\_GenKeypair}(Seed_B)$；
