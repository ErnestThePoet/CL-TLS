...
#define GENERATE_KE_KEY_RANDOM                                   \
    ...
    {                                                            \
        X25519_keypair(self_ke_public_key, self_ke_private_key); \
                                                                 \
        BIGNUM *self_handshake_random_bn = BN_new();             \
        if (self_handshake_random_bn == NULL)                    \
        {                                                        \
            LogError("[%s] Memory allocation for "               \
                     "|self_handshake_random_bn| failed",        \
                     current_stage);                             \
            exit(EXIT_FAILURE);                                  \
        }                                                        \
                                                                 \
        if (!BN_rand(self_handshake_random_bn,                   \
                     CLTLS_HANDSHAKE_RANDOM_LENGTH * 8,          \
                     BN_RAND_TOP_ANY,                            \
                     BN_RAND_BOTTOM_ANY))                        \
        {                                                        \
            ...
            BN_free(self_handshake_random_bn);                   \
            HANDSHAKE_SEND_ERROR_STOP_NOTIFY_FREE_RETURN_FALSE(  \
                CLTLS_ERROR_INTERNAL_EXECUTION_ERROR);           \
        }                                                        \
                                                                 \
        if (!BN_bn2bin_padded(self_handshake_random,             \
                              CLTLS_HANDSHAKE_RANDOM_LENGTH,     \
                              self_handshake_random_bn))         \
        {                                                        \
